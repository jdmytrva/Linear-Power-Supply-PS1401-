...
void ENC_PollEncoder(void)
{
	...
	if (!GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_11)) {SetBit(currentState,0);}
	if (!GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_8)) {SetBit(currentState,1);}
...
int main(void)
{
    ...
    while(1)
    {
    	...
    	Print_to_USART1_d(Button,"Enc:",0);
    	if (!GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_4)) Print_to_USART1("OK");
		switch (Button)
		{
			case KEY_BACK:
				Menu_Navigate(MENU_PREVIOUS);
